...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        x_padded = torch.nn.functional.pad(x, (pad, pad, pad, pad)) if pad > 0 else x.clone()

        out = torch.zeros((N, F, H_out, W_out), dtype=x.dtype, device=x.device)

        for n_img in range(N):
            for n_filter in range(F):
                for x_idx in range(H_out):
                    for y_idx in range(W_out):
                        vert_start = x_idx * stride
                        vert_end = vert_start + HH
                        horiz_start = y_idx * stride
                        horiz_end = horiz_start + WW

                        window = x_padded[n_img, :, vert_start:vert_end, horiz_start:horiz_end]
                        out[n_img, n_filter, x_idx, y_idx] = (window * w[n_filter]).sum() + b[n_filter]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
            ...
            for f in range(F):
                for i in range(H_out):
                    for j in range(W_out):
                        ...
                        dx_padded[n, :, vert_start:vert_end, horiz_start:horiz_end] += dout_val * w[f]
            ...
            for c in range(C):
                for i in range(H_out):
                    for j in range(W_out):
                        ...
                        window = x[n, c, vert_start:vert_end, horiz_start:horiz_end]
                        mask = (window >= window.max() - 1e-12).to(x.dtype)
...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        conv_out, conv_relu_pool_cache = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
        lin_relu_out, lin_relu_cache = Linear_ReLU.forward(conv_out, W2, b2)
        scores, lin_cache = Linear.forward(lin_relu_out, W3, b3)
        ...
        loss = loss + 0.5 * self.reg * (W1.norm()**2 + W2.norm()**2 + W3.norm()**2)
        
        dout, dW3, db3 = Linear.backward(dout=grad_softmax, cache=lin_cache)
        dout, dW2, db2 = Linear_ReLU.backward(dout, lin_relu_cache)
        _, dW1, db1 = Conv_ReLU_Pool.backward(dout, conv_relu_pool_cache)
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    for i in range(self.num_layers - 1):
      if weight_scale == 'kaiming':
        self.params['W' + str(i+1)] = kaiming_initializer(Din=C, Dout=num_filters[i], K=3, device=device, dtype=dtype)
      else:
      ...
      if i in max_pools:
        ...
        W //= 2
    if weight_scale == 'kaiming':
      self.params['W' + str(self.num_layers)] = kaiming_initializer(Din=C*H*W, Dout=num_classes, device=device, dtype=dtype)
    else:
      self.params['W' + str(self.num_layers)] = torch.randn(C*H*W, num_classes, device=device, dtype=dtype) * weight_scale
  ...
  def loss(self, X, y=None):
    ...
    dout, dW, db = Linear.backward(grad_softmax, cache_list.pop())
    ...
    for i in range(self.num_layers - 2, -1, - 1):
      if self.batchnorm:
        if i in self.max_pools:
          dout, dW, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dout, cache_list.pop())
          grads['gamma' + str(i+1)] = dgamma
          grads['beta' + str(i+1)] = dbeta
        else:
          dout, dW, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dout, cache_list.pop())
          grads['gamma' + str(i+1)] = dgamma
          grads['beta' + str(i+1)] = dbeta
      else:
        if i in self.max_pools:
          dout, dW, db = Conv_ReLU_Pool.backward(dout, cache_list.pop())
        else:
          dout, dW, db = Conv_ReLU.backward(dout, cache_list.pop())
...
class BatchNorm(object):
  ...
  def backward(dout, cache):
    ...
    dvar = (-0.5 * (dxhat * (x - mean)).sum(dim=0) * (var + eps)**(-1.5))
    dmean = (-dxhat / torch.sqrt(var + eps)).sum(dim=0) + dvar * (-2.0 / N * (x - mean).sum(dim=0))
    dx = dxhat / torch.sqrt(var + eps) + dvar * (2.0 / N * (x - mean)) + dmean / N
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
  ...
  def backward_alt(dout, cache):
    ...
    dstd_dev = torch.sum(dxhat * (x - mean), dim=0) * (-1) / std_dev**2.0
    dvar = 0.5 / std_dev * dstd_dev
    dmu = -1.0 / std_dev * dxhat.sum(dim=0) + dvar * (-2.0 / N * (x - mean).sum(dim=0))
    dx = dxhat / std_dev + dvar * (2.0 / N * (x - mean)) + dmu / N
