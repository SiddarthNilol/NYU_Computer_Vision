...
class Trainer(object):
    ...
    def train(self):
        ...
        for self.step in tqdm(range(start_step, self.train_num_steps), desc="steps"):
            ...
            if (self.step + 1) % self.save_and_sample_every == 0:
                ...
                # save model checkpoint for this milestone so it can be loaded later
                try:
                    self.save(itrs=milestone)
                except Exception as e:
                    print(f"Warning: failed to save model checkpoint at milestone {milestone}: {e}")
    ...
    def visualize_diffusion(
        ...
        for t in t_visualize:
            ...
            grid = utils.make_grid(img, nrow=8)
            img_forward.append(grid)

        # visualize backward diffusion process
        img_backward = []

        t_full = torch.full((data_1.shape[0],), self.model.num_timesteps - 1, device=data_1.device, dtype=torch.long)
        img = self.model.q_sample(data_1, t_full, noise=torch.randn_like(data_1))
        ...
        for t in range(self.model.num_timesteps - 1, -1, -1):
            ...
            if t in t_visualize:
                ...
                img_vis = (img_vis + 1) / 2
                grid_vis = utils.make_grid(img_vis, nrow=8)
                img_backward.append(grid_vis)
