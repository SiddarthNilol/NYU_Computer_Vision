...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):
        ...
        for k in range(K):
            weights[k] = torch.matmul(query[n, k], key[n].T) / (M ** 0.5)
...
def scaled_dot_product_no_loop_batch(
    ...
    if mask is not None:
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        Q_Kt = torch.bmm(query, key.transpose(1, 2)) / (M ** 0.5)
        Q_Kt = Q_Kt.masked_fill(mask, -1e9)
    # Replace "pass" statement with your code
    else:
        Q_Kt = torch.bmm(query, key.transpose(1, 2)) / (M ** 0.5)
    weights_softmax = F.softmax(Q_Kt, dim=-1)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        nn.init.uniform_(self.linear.weight, - (6 / (num_heads * dim_out + dim_in)) ** 0.5, (6 / (num_heads * dim_out + dim_in)) ** 0.5)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        nn.init.uniform_(self.linear1.weight, - (6 / (inp_dim + hidden_dim_feedforward)) ** 0.5, (6 / (inp_dim + hidden_dim_feedforward)) ** 0.5)
        nn.init.uniform_(self.linear2.weight, - (6 / (hidden_dim_feedforward + inp_dim)) ** 0.5, (6 / (hidden_dim_feedforward + inp_dim)) ** 0.5)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N, K = seq.shape
    mask = torch.triu(torch.ones((N, K, K), dtype=torch.bool), diagonal=1) # **to_double_cuda
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return mask
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    position = torch.arange(K).unsqueeze(1)
    pos_encoding = position / K
    y = pos_encoding.repeat(1, M).unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    positions = torch.arange(K, dtype=torch.float).unsqueeze(1)         
    dims = torch.arange(M, dtype=torch.float).unsqueeze(0)              
    angle_rates = 1.0 / (10000.0 ** ((2 * (dims // 2)) / M))            
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        mask = get_subsequent_mask(ans_b[:, :-1]).to(ques_b.device)
